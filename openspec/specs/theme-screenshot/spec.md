# Spec: Theme Screenshot Generation

## Purpose
Enable theme developers to generate a PNG preview image (`preview.png`) for their theme using a dedicated CLI command. The screenshot captures the rendered HTML at a fixed retina-quality viewport, using `sample.yml` (or `cv.yml` as fallback) as CV source data.
